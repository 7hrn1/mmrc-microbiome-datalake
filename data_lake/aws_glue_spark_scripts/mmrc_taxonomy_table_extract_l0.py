...
bucket_name = "mmrc-taxonomy-tables"
...
table_name = "mmrc_taxonomy_table"
...
def main():
    ...
    try:
        ...
        tax_dyf = glueContext.create_dynamic_frame.from_options(
            format_options={
                "withHeader": True
            },
            connection_type="s3",
        ...
        #Before extraction of taxonomy id column, check if any nulls or "" values
        tax_id_col_header = tax_df.schema[0].name
        # Check for null, empty string, or spaces, and raise an error
        if tax_df.filter((F.col(tax_id_col_header).isNull()) | (F.trim(F.col(tax_id_col_header)) == "")).count() > 0:
            raise ValueError(f"Error: Missing/Incorrect Taxonomy ID's found!")
        #1. Extract and clean taxonomy id column
        tax_df = tax_df.withColumn("Taxonomy_ID_Cleaned", F.lower(tax_df[tax_id_col_header]))
        tax_df = tax_df.drop(tax_id_col_header)
        tax_df = tax_df.withColumnRenamed("Taxonomy_ID_Cleaned", "Taxonomy_ID")

        #2. Change all column names to lowercase
        lowercased_cols = [c.lower() for c in tax_df.columns]
        cleaned_cols_df = tax_df.toDF(*lowercased_cols)

        #3. Rearrange such that sample id is in front
        cleaned_cols_df =  cleaned_cols_df.select("taxonomy_id", *[f"`{col}`" for col in cleaned_cols_df.columns if col != "taxonomy_id"])
