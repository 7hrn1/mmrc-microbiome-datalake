...
bucket_name = "mmrc-bacterial-abundance"
input_path = job_params['full_file_path']
file_name = job_params["file_name"]

#Configure sam table details
catalog_name = "glue_catalog"
database_name = "mmrc_microbiome"
table_name = "mmrc_bacterial_abundance_sam"
full_table_name = f"""{catalog_name}.{database_name}.{table_name}"""

#Configure tax table details
catalog_name_t = "glue_catalog_t"
table_name_t = "mmrc_bacterial_abundance_tax"
full_table_name_t = f"""{catalog_name_t}.{database_name}.{table_name_t}"""
...
def main():
    ...
    try:
        ...
        bac_abund_dyf = glueContext.create_dynamic_frame.from_options(
            format_options={
                "withHeader": True
            },
            connection_type="s3",
        ...
        bac_abund_df = bac_abund_dyf.toDF()
        
        #1. Extract 4-digit sample number and standardise format to be SampleXXXX
        sample_id_col_header = bac_abund_df.schema[0].name
        cleaned_sample_id_df = bac_abund_df.withColumn(
            "Sample_ID_Extracted", 
        ...
        #3. Cast all numeric columns to double double
        numeric_cols = [c for c in cleaned_cols_df.columns if c != "sample_id"]
        casted_columns = [F.col("sample_id")] + \
                 [F.col(f"`{c}`").cast("double").alias(c) for c in numeric_cols]
        cleaned_cols_df = cleaned_cols_df.select(*casted_columns)
        
        #Fill null values as 0
        zeroed_df = cleaned_cols_df.fillna(0)

        #Copy df for sam table writing
        sam_df = zeroed_df
        
        #Add source and batch date,month, year columns for partitioning
        sam_df = sam_df.withColumn("source_file", F.lit(f"{file_name}")) \
        ...
        #Setting up column lists to be transposed
        zeroed_df_cols = zeroed_df.columns
        id_column = zeroed_df_cols[0]  # First Column is sample id
        value_columns = zeroed_df_cols[1:]
        
        #Transpose
        transposed_df = zeroed_df.select(
            F.col(wrap_expr(id_column)),
            F.explode(F.array([
                F.struct(F.lit(wrap_expr(col_name)).alias("key"), F.col(wrap_expr(col_name)).alias("value"))
                for col_name in value_columns
            ])).alias("_tmp")
        ).select(
            F.col(wrap_expr(id_column)),
            F.col("_tmp.key").alias("taxonomy_id"),
            F.col("_tmp.value").alias("value")
        )
        
        # Step 2: Pivot the stacked dataframe
        transposed_df = transposed_df.groupBy("taxonomy_id").pivot(wrap_expr(id_column)).agg(
            F.first("value")
        )
        #Replace any backtick
        transposed_df = transposed_df.withColumn("taxonomy_id", F.regexp_replace(F.col("taxonomy_id"), "`", ""))
        
        #Add source and batch date,month, year columns for partitioning
        tax_df = transposed_df.withColumn("source_file", F.lit(f"{file_name}")) \
            .withColumn("batch_day", F.lit(datetime.now().strftime('%d'))) \
            .withColumn("batch_month", F.lit(datetime.now().strftime('%m'))) \
            .withColumn("batch_year", F.lit(datetime.now().strftime('%Y')))
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            future_to_name = {
                executor.submit(iceberg_table_transform, full_table_name, sam_df): "sam",
                executor.submit(iceberg_table_transform, full_table_name_t, tax_df): "tax"
            }
        ...
        if not results.get("sam") or not results.get("tax"):
            raise Exception("One or both of the Iceberg table transforms failed.")
